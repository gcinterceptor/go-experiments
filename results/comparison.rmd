...
al1.exp.nogci <- filter(al1.exp.gci, status == 200)
```
...
I opened a little my research and found papers about model validation in other areas. In particular, [this](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.532.2506&rep=rep1&type=pdf) provides a great range of statistics and explain those. Furthermore, I found the [gof function in the hydroGOF package](https://www.rforge.net/doc/packages/hydroGOF/gof.html). 

```{r}
gof(sample(al1.sim.gci$latency, 30000), sample(al1.exp.gci$request_time, 30000))
```
