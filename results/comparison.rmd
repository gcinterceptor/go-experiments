...
* To model the time that a request stay at a server, chose some value randomly from a experimental log may be enough.
* To model the GCI behavior, reproduce how many requests were processed until GCI start shedding and reproduce how many request should be shedded until GCI stop shedding may be enough.
...
* If each request was done or not. It means if the request was processed in some server or refused for all.
